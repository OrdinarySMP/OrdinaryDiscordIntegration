...
public class DiscordChatMessageSender {
    ...
    public DiscordChatMessageSender(JDAWebhookClient webhookClient, TextChannel channel, Config config, String message, ServerPlayerEntity sender) {
    ...
    private boolean canUpdateMessage() {
        return this.readyFuture == null
                || !this.readyFuture.isDone()
    ...
    private void sendMessageToDiscord() {
        ...
        this.readyFuture = this.channel.sendMessage(this.getMessage())
                .submit()
                .thenApply(Message::getIdLong)
                .thenAccept(this::updateMessage)
    ...
    private void editDiscordMessage() {
        ...
        this.channel.editMessageById(this.messageId, getMessage() + displayedCount)
                .submit()
                .exceptionally(this::handleFailure);
    }
    ...
    private void sendAsWebhook() {
        ...
        WebhookMessage msg = new WebhookMessageBuilder()
                .setUsername(sender.getName().getLiteralString())
                .setAvatarUrl(avatarUrl)
                .setContent(message)
                .build();
        this.readyFuture = this.webhookClient.send(msg)
